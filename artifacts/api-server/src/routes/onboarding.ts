...
import { parseVendorApplicationStatusLookup } from "../lib/onboarding-validation.ts";
...
onboardingRouter.get(
  "/v1/vendor/applications/status",
  createRateLimiter({
    windowMs: 15 * 60 * 1000,
    max: 20,
    message: "Too many status checks from this address. Please try again later.",
  }),
  async (req, res) => {
    const lookup = parseVendorApplicationStatusLookup(req.query);
    const unavailable = () => {
      error(res, 404, "APPLICATION_STATUS_UNAVAILABLE", "Application status is unavailable.");
    };
    if (!lookup) {
      unavailable();
      return;
    ...
    const application = await db.query.vendorApplications.findFirst({
      where: and(eq(vendorApplications.id, lookup.id), eq(vendorApplications.email, lookup.email)),
    });
    if (!application) {
      unavailable();
      return;
