...
export default function VerifyScreen() {
  const colors=useColors(), insets=useSafeAreaInsets(); const {signUp,fetchStatus}=useSignUp(); const [code,setCode]=useState(''); const [message,setMessage]=useState(''); const [isVerifying,setIsVerifying]=useState(false); const [isResending,setIsResending]=useState(false);
  const loading=fetchStatus==='fetching'||isVerifying||isResending;
  const verify=async()=>{if(isVerifying)return; setMessage(''); setIsVerifying(true); try { const {error}=await signUp.verifications.verifyEmailCode({code}); if(error)return setMessage(verificationErrorMessage(error,'That code did not work. Please try again.')); if(signUp.status!=='complete')return setMessage('Please check the code and try again.'); await signUp.finalize({navigate:()=>router.replace('/(tabs)')}); } catch (error) { setMessage(verificationErrorMessage(error,'We could not verify your email. Please try again.')); } finally { setIsVerifying(false); }};
  const resend=async()=>{if(isResending)return; setMessage(''); setIsResending(true); try { const {error}=await signUp.verifications.sendEmailCode(); if(error)return setMessage(verificationErrorMessage(error,'We could not send a new verification code. Please try again.')); setMessage('A new verification code was sent.'); } catch (error) { setMessage(verificationErrorMessage(error,'We could not send a new verification code. Please try again.')); } finally { setIsResending(false); }};
