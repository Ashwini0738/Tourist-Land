      ...
      <Modal visible={Boolean(pickerItem)} transparent animationType="slide" onRequestClose={() => setPickerItem(null)}>
        <Pressable style={styles.modalBackdrop} onPress={() => setPickerItem(null)}>
          <Pressable style={[styles.sheet, { backgroundColor: colors.background }]} onPress={(event) => event.stopPropagation()}>
            ...
            {tripsQuery.isLoading ? <View style={styles.noTrips}><ActivityIndicator color={colors.primary} /><Text style={[styles.noTripsText, { color: colors.mutedForeground }]}>Loading your trips…</Text></View> : tripsQuery.isError ? <View style={styles.noTrips}><Feather name="alert-circle" size={22} color={colors.destructive} /><Text style={[styles.noTripsTitle, { color: colors.foreground }]}>Trips could not load</Text><Text style={[styles.noTripsText, { color: colors.mutedForeground }]}>Try again, or open My Trips to create a trip directly.</Text><View style={styles.pickerActions}><Pressable accessibilityRole="button" onPress={() => void tripsQuery.refetch()} style={[styles.pickerSecondary, { borderColor: colors.border }]}><Text style={[styles.pickerSecondaryText, { color: colors.primary }]}>Try again</Text></Pressable><Pressable accessibilityRole="button" onPress={() => { setPickerItem(null); router.push('/trips' as any); }} style={[styles.button, { backgroundColor: colors.primary, marginTop: 0 }]}><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>Open My Trips</Text></Pressable></View></View> : !activeTrips.length ? <View style={styles.noTrips}><Feather name="calendar" size={22} color={colors.primary} /><Text style={[styles.noTripsTitle, { color: colors.foreground }]}>Create your first trip</Text><Text style={[styles.noTripsText, { color: colors.mutedForeground }]}>Give your saved places a plan, then come back when you’re ready.</Text><Pressable onPress={() => { setPickerItem(null); router.push('/trips' as any); }} style={[styles.button, { backgroundColor: colors.primary }]}><Text style={[styles.buttonText, { color: colors.primaryForeground }]}>Create a trip</Text></Pressable></View> : activeTrips.map((trip) => <Pressable key={trip.id} testID={`saved-trip-option-${trip.id}`} accessibilityRole="button" disabled={Boolean(addingToTrip)} onPress={() => void addToTrip(trip)} style={[styles.tripOption, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={styles.tripOptionCopy}><Text style={[styles.tripOptionTitle, { color: colors.foreground }]}>{trip.title}</Text><Text style={[styles.tripOptionMeta, { color: colors.mutedForeground }]}>{trip.items.length} planned item{trip.items.length === 1 ? '' : 's'}</Text></View>{addingToTrip === trip.id ? <ActivityIndicator color={colors.primary} /> : <Feather name="plus-circle" size={21} color={colors.primary} />}</Pressable>)}
          </Pressable>
        </Pressable>
      </Modal>
    </>
...
const styles = StyleSheet.create({
  ...
  pickerActions: { flexDirection: 'row', alignItems: 'center', gap: 9, marginTop: 15 },
  pickerSecondary: { borderWidth: 1, borderRadius: radii.pill, paddingHorizontal: 14, paddingVertical: 10 },
  pickerSecondaryText: { fontSize: 12, fontWeight: '800' },
