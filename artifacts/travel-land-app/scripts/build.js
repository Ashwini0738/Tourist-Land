...
const metroPortSearchLimit = 100;
...
function getRequestedMetroPort() {
  const rawPort = process.env.EXPO_METRO_PORT;
  if (!rawPort) {
    return defaultMetroPort;
  }

  const port = Number(rawPort);
  if (!Number.isInteger(port) || port <= 0 || port > 65535) {
    throw new Error(
      `Invalid EXPO_METRO_PORT value: "${rawPort}". Set it to an available TCP port between 1 and 65535.`,
    );
  }
...
async function findAvailableMetroPort() {
  ...
  const portCount = process.env.EXPO_METRO_PORT
    ? 1
    : metroPortSearchLimit;
...
function getMetroUrl(pathname) {
  ...
  return `http://127.0.0.1:${metroPort}${normalizedPath}`;
}
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  if (selectedPort === null) {
    const requestedPort = getRequestedMetroPort();
    const portDescription = process.env.EXPO_METRO_PORT
      ? `${requestedPort}`
      : `${requestedPort}-${requestedPort + metroPortSearchLimit - 1}`;
    throw new Error(
      `No available Metro port found in ${portDescription}. ` +
        'Stop the process using that port or set EXPO_METRO_PORT to a free port.',
    );
  }
